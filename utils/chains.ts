...
import { ChainInfoWithExplorer, SimplifiedChainInfo } from 'types/chains';
...
export function createKeplrChainInfos(chainInfo: SimplifiedChainInfo): ChainInfoWithExplorer {
	...
	for (const currency of chainInfo.currencies) {
		if (currency.isFeeCurrency) feeCurrencies.push(currency);

		if (currency.isStakeCurrency && stakeCurrency === undefined) stakeCurrency = currency;
		else if (currency.isStakeCurrency) throw new Error(`There cannot be more than one stake currency for ${chainInfo.chainName}`);
	}

	if (stakeCurrency === undefined) throw new Error(`Did not specify a stake currency for ${chainInfo.chainName}`);

	if (feeCurrencies.length === 0) throw new Error(`Did not specify any fee currencies for ${chainInfo.chainName}`);
